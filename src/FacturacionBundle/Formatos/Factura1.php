...
namespace FacturacionBundle\Formatos;
...
class Factura1 extends \FPDF_FPDF {
    ...
    public function Generar($em, $codigoFactura) {        
        ...
        $valor = round($arFactura->getValorTotal());
//        $strLetras = \GeneralBundle\MisClases\Funciones::devolverNumeroLetras($valor);
...
//        $arConfiguracionTurno = self::$em->getRepository('BrasaTurnoBundle:TurConfiguracion')->find(1);
        $arFactura = new \FacturacionBundle\Entity\Factura();
        $arFactura = self::$em->getRepository('FacturacionBundle:Factura')->find(self::$codigoFactura);
        $this->SetXY(10, 10);
...
//        $this->MultiCell(140, 3, $arConfiguracionTurno->getInformacionResolucionDianFactura(), 0, 'L');
        ...
        $Datos = array($arFactura->getClienteRel()->getPrimerNombre(),
            $arFactura->getClienteRel()->getNumDocumento(),
...
//            $arFactura->getSoporte());
        ...
        $this->Cell(30, 3, $arFactura->getNumeroDocumento(), 0, 0, 'R'); 
        $this->SetY(27);
        $this->SetFont('Arial', '', 8);        
        foreach ($List1 as $col) {
            $this->SetX(175);
...
//                $arFactura->getClienteRel()->getBarrio());                        
    ...
    public function Body($pdf) {
        $arFacturaDetalles = new \FacturacionBundle\Entity\FacturaDetalle();
        $arFacturaDetalles = self::$em->getRepository('FacturacionBundle:FacturaDetalle')->findBy(array('codigoFacturaFk' => self::$codigoFactura));
        $pdf->SetX(10);
        $pdf->SetFont('Arial', '', 7);
        foreach ($arFacturaDetalles as $arFacturaDetalle) {
           
                $arFacturaDetalle->getArticuloRel()->getNombreArticulo();
            }
            $pdf->Cell(15, 4, $arFacturaDetalle->getCodigoFacturaDetallePk(), 1, 0, 'L');
//            $pdf->Cell(125, 4, $strDetalle, 1, 0, 'L');
            $pdf->Cell(10, 4, $arFacturaDetalle->getArticuloRel()->getNombreArticulo(), 1, 0, 'L');
            $pdf->Cell(15, 4, number_format($arFacturaDetalle->getCantidad(), 0, '.', ','), 1, 0, 'R');
            $pdf->Cell(30, 4, number_format($arFacturaDetalle->getVrSubTotal(), 0, '.', ','), 1, 0, 'R');
            $pdf->Ln();
...
//        $arConfiguracion = self::$em->getRepository('BrasaTurnoBundle:TurConfiguracion')->find(1);
        ...
        $totales2 = array(number_format($arFactura->getVrSubtotal(), 0, '.', ','),
            number_format($arFactura->getVrBaseIva(), 0, '.', ','),
...
//            number_format($arFactura->getVrRetencionIva(), 0, '.', ','),
            number_format($arFactura->getValorTotal(), 0, '.', ',')
        );
